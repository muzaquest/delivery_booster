...
def tab_restaurant_analysis():
        ...
        with col1:
                label = st.selectbox('Ресторан', list(rest_map.keys()), key='restaurant_selectbox')
        rest_id = rest_map[label]
        rest_name = label.split(' (ID')[0]

        presets = _period_presets()
        preset = st.selectbox('Период (пресеты)', list(presets.keys()), key='restaurant_analysis_preset')
        start_default, end_default = presets[preset]
        col1, col2 = st.columns(2)
        with col1:
                start_date = st.date_input('Начало', start_default, key='restaurant_start_date')
        with col2:
                end_date = st.date_input('Окончание', end_default, key='restaurant_end_date')
...
def tab_base_analysis():
        ...
        col1, col2 = st.columns(2)
        with col1:
                start_date = st.date_input('Начало', start_default, key='base_start_date')
        with col2:
                end_date = st.date_input('Окончание', end_default, key='base_end_date')
